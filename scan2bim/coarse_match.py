...
refined by trimmed re-association, and scored. Nothing is fitted globally: the walk is
...
which evidence works - tests/test_coarse_match.py pins each line below:
  ...
  * MORE LEGS ALONE DOES NOT. A 3-leg U and a 4-leg staircase still tie (margin 0.0 /
    ...
    leg lengths bound the scale from one side and never pin it.
...
def _build(chi: int, theta_deg: float, s_f: float, s_h: float, in_dir_flip: np.ndarray,
    ...
    yaw is whatever rotation carries the SCALED incoming walk direction onto the plan
    incoming direction (computed through the actual tensor, so it stays exact even when
    the walk's corner is not exactly 90 deg), and the translation anchors the walk's
    corner onto the plan's corner, plus the trimmed correction `dt`."""
    A = anisotropic_scale_tensor(np.deg2rad(theta_deg), s_f, s_h)
    yaw = _wrap180(_ang(plan_in_dir) - _ang(A @ in_dir_flip))
...
def _hypotheses(R: dict, P: dict, tol: dict, aniso_band) -> list:
        ...
        for pc in P["approaches"]:
            ...
            for chi in (1, -1):
                ...
                if not (aniso_band[0] <= s_f / s_h <= aniso_band[1]):
                    continue
                seeds.append({"chi": chi, "theta": theta, "s_f": s_f, "s_h": s_h,
...
def coarse_match(skel: dict, traj_xz, door_s=None, gates=None, tol=None, weights=None,
                 ...
                 scale_factors=SCALE_FACTORS, recon_kwargs=None) -> dict:
    ...
    seeds = _hypotheses(R, P, t, aniso_band)
